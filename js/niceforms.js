...
function inputRadio(el) { //extent Radio buttons
	...
	el.dummy.ref = el;
	if(isIE == false) {
		el.dummy.style.left = $(el).position().left + 'px';
		el.dummy.style.top = $(el).position().top + 'px';
	}
	else {
		el.dummy.style.left = $(el).position().left + 4 + 'px';
		el.dummy.style.top = $(el).position().top + 0 + 'px';
...
function selects(el) {
	...
	el.dummy.setAttribute('ref_name', el.name);
	el.left = document.createElement('img');
	el.dummy.ref = el;
	el.left.src = imagesPath + "0.png";
	...
	el.flag = false; //указывает на то что клик был произведен на элементе списка
	if(el.options.length == 0){
	    el.options[0] = new Option("", "");
	}
	var editable = el.getAttribute('is_editable');
    if(editable == "true"){
        el.txt = document.createElement('input');
        el.txt.style.cssText = "border: 0px; margin-top:1px; padding-left:0px; background:rgba(0,0,0,0); box-shadow:none; width:95%; color:black;";
        el.txt.setAttribute("autocomplete","off");
        el.txt.id = "cb_editable";
        el.txt.type = "text";
        el.txt.value = el.options[el.selectedIndex].text;
        el.txt.onkeydown = el.txt.onkeyup = el.txt.onkeypress = function(e){
            var options = el.opt.childNodes;
            var index_result = -1;
            var search_text = (this.value + getChar(e));
            for(var index = 0; index < options.length; index++){
                if(el.opt.children[index].textContent.toLowerCase().indexOf(search_text.toLowerCase()) == -1)
                    el.opt.children[index].style.display = "none";
                else{
                    if(index_result == -1) index_result = index;
                    el.opt.children[index].style.display = "block";
                }
            }
            if(index_result == -1) index_result = 0;
            el.txt.data = el.opt.children[index_result].textContent;
            el.selectedIndex = index_result;
            for(var w = 0; w < el.options.length; w++) {el.options[w].lnk.className = "";}
            el.options[index_result].lnk.className = "NFOptionActive";
        };
    } else {
        el.txt = document.createTextNode(el.options[el.selectedIndex].text);
    }
	el.bg = document.createElement('div');
	el.bg.className = "NFSelectTarget";
	el.bg.style.display = "none";
	el.bg.style.width = '1000px';
	...
	el.find_input = null;
	el.setactive = function(value){
	    if(value == "false")
	        this.right.style.color = "#B3B0B0";
	    else
	        this.right.style.color = "black";
	...
	el.init = function(pos) {
		...
		var is_editable = this.getAttribute('is_editable');
		for(var q = 0; q < this.options.length; q++) {
			...
			this.options[q].lnk.onclick = function() {
				this._onclick();
				if(is_editable == "true")
				    this.ref.txt.value = this.ref.options[this.pos].text;
				else
				...
				this.ref.flag = true;
				this.ref.dummy.setAttribute('selected_value',this.ref.options[this.pos].value);
			}
		}
		if(this.options.length != 0)
		    this.dummy.setAttribute('selected_value',this.options[0].value);
		if(this.options.selectedIndex) {
		    if(is_editable == "true")
                this.txt.value = this.options[this.options.selectedIndex].text;
            else
			...
			this.dummy.setAttribute('selected_value',this.options[this.options.selectedIndex].value);
		}
		this.dummy.style.zIndex = 999 - pos;
		this.parentNode.insertBefore(this.dummy, this);
		this.className = "NFhidden";

	    var active = this.getAttribute('is_active');
        if(active == "false")
           this.right.style.color = "#B3B0B0";
	}
	el.unload = function() {
	...
	el.dummy.onclick = function() {
	    var active = el.getAttribute('is_active');
        if(active == null)
            active = "true";
        if(active == "false")
            return;

        var is_editable = el.getAttribute('is_editable');
        if(is_editable == "true" && el.flag == false)
            el.txt.value = "";
        else
            el.flag = false;

		var max_height = el.getAttribute('select_height');
        if(max_height == null)
            max_height = selectMaxHeight;
        else
            max_height = max_height * 34;
		...
		if(this.ref.bg.style.display == "none") {
			...
			this.ref.opt.style.maxHeight = max_height + 4 + "px";
            for(var index = 0; index < this.ref.options.length; index++){
                this.ref.opt.children[index].style.display = "block";
                if(this.ref.getAttribute('options_font_size') != null)
                    this.ref.opt.children[index].style.fontSize = this.ref.getAttribute('options_font_size');
            }
		}
		else {
		    this.ref.bg.style.display = "none";
		    this.ref.txt.value = this.ref.options[this.ref.options.selectedIndex].text;
		}

		if(this.ref.opt.offsetHeight > max_height) {
		...
		for(var q = 0; q < allDivs.length; q++){
            if((allDivs[q].className == "NFSelectTarget") && (allDivs[q] != el.bg)) {
                allDivs[q].style.display = "none";
            }
        }
        if(el.bg.style.display == "none") {
        	el.bg.style.display = "block";
        	el.opt.style.maxHeight = max_height + 4 + "px";
            for(var index = 0; index < el.options.length; index++){
                el.opt.children[index].style.display = "block";
                if(el.getAttribute('options_font_size') != null)
                    el.opt.children[index].style.fontSize = el.getAttribute('options_font_size');
            }
        }
        else {
            el.bg.style.display = "none";
            el.txt.value = el.options[el.options.selectedIndex].text;
        }

        if(el.opt.offsetHeight > max_height) {
        	el.bg.style.width = el.offsetWidth - selectRightWidthScroll + 33 + 'px';
        	el.opt.style.width = el.offsetWidth - selectRightWidthScroll + 'px';
        }
        else {
        	el.bg.style.width = el.offsetWidth - selectRightWidthSimple + 33 + 'px';
        	el.opt.style.width = el.offsetWidth - selectRightWidthSimple + 'px';
        }
	}
	el.bg.onmouseout = function(e) {
		...
		if((reltg!=null && reltg.nodeName != null) &&((reltg.nodeName == 'A') || (reltg.nodeName == 'LI') || (reltg.nodeName == 'UL'))) return;
        if(reltg!=null && reltg.nodeName != null && ((reltg.nodeName == 'DIV') || (reltg.className == 'NFSelectTarget'))) return;
        if(reltg!=null && reltg.id == "cb_editable") return;
		else{
		    if(this.className != "NFSelect"){
                this.parentElement.ref.txt.value = this.parentElement.ref.options[this.parentElement.ref.options.selectedIndex].text;
                this.parentElement.ref.bg.style.display = "none";
            } else {
                this.ref.txt.value = this.parentElement.ref.options[this.parentElement.ref.options.selectedIndex].text;
                this.ref.bg.style.display = "none";
            }
        }
	}
